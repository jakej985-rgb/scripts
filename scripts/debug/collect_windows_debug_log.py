...
FULL_LOG_FILE = REPO_ROOT / "logs_windows.txt"
ERROR_LOG_FILE = REPO_ROOT / "error_log_windows.txt"
...
def collect_logs():
    ...
    try:
        ...
        with open(FULL_LOG_FILE, "w", encoding="utf-8") as full_f, \
            ...
            header = f"M3TAL SYSTEM DEBUG BUNDLE (WINDOWS)\nGenerated: {ts}\nRepo Root: {REPO_ROOT}\n" + ("=" * 80) + "\n\n"
            full_f.write(header)
            err_f.write(f"M3TAL ERROR LOG (WINDOWS)\nGenerated: {ts}\n" + ("=" * 80) + "\n\n")
            ...
            sys_info += run_cmd(["ipconfig", "/all"], "Network Configuration")
            write_dual(sys_info)
